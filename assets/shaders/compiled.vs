...
float zwl_seg(vec2 p, vec2 a, vec2 b) {
    vec2 pa = p - a;
    vec2 ba = b - a;
...
float zwl_stroke(vec3 p, vec2 a, vec2 b, float radius, float depth) {
    float flat = zwl_seg(p.xy, a, b) - radius;
    float side = abs(p.z) - depth;
    return max(flat, side);
}

float zwl_logo_sdf(vec3 p) {
    p.xy *= 1.12;
    p.y += 0.015;
    float r = 0.055;
    float d = 10.0;
    ...
    // Z
    d = min(d, zwl_stroke(p, vec2(-0.74, 0.30), vec2(-0.30, 0.30), r, 0.105));
    d = min(d, zwl_stroke(p, vec2(-0.30, 0.30), vec2(-0.74,-0.30), r, 0.105));
    d = min(d, zwl_stroke(p, vec2(-0.74,-0.30), vec2(-0.30,-0.30), r, 0.105));

    // W
    d = min(d, zwl_stroke(p, vec2(-0.22, 0.30), vec2(-0.12,-0.30), r, 0.105));
    d = min(d, zwl_stroke(p, vec2(-0.12,-0.30), vec2( 0.00, 0.08), r, 0.105));
    d = min(d, zwl_stroke(p, vec2( 0.00, 0.08), vec2( 0.12,-0.30), r, 0.105));
    d = min(d, zwl_stroke(p, vec2( 0.12,-0.30), vec2( 0.22, 0.30), r, 0.105));

    // L
    d = min(d, zwl_stroke(p, vec2( 0.34, 0.30), vec2( 0.34,-0.30), r, 0.105));
    d = min(d, zwl_stroke(p, vec2( 0.34,-0.30), vec2( 0.74,-0.30), r, 0.105));

    float bounds = length(vec3(p.xy, p.z * 1.4)) - 0.86;
    return max(d, bounds);
}

float logo_sdf(vec3 p) {
    return zwl_logo_sdf(p);
}
